...
const Publish = () => {
  ...
  const [selectedTag, setSelectedTag] = useState('#生活') // 新增状态，默认选中
    ...
    <div className='publish-all'>
      ...
      {previewImage && (
        ...
        <div className="publish-tags">
          ...
          <span
            className={`publish-tag ${selectedTag === '#生活' ? 'selected' : ''}`}
            onClick={() => handleTagClick('#生活')}
          >
            #学习讨论
          </span>
          <span
            className={`publish-tag ${selectedTag === '#爱情' ? 'selected' : ''}`}
            onClick={() => handleTagClick('#爱情')}
          >
            #竞赛分享
          </span>
        </div>
